define(['jsbn', 'jquery'], function (jsbn, $) {
	/* 5 */
	var ASN = (function() {
		...
		function _inherits(subClass, superClass) {
			...
			// FIXME Object.create IE8不兼容, 非对称加密时不使用到该类
			if (!Object.create) {
				return;
			}
		...
		return {
			...
			encodeDer: function encodeDer(r, s) {
				var derR = new DERInteger({
					bigint: r
				});
				var derS = new DERInteger({
					bigint: s
				});
	...
	var EC = (function() {
		...
		var ECFieldElementFp = function() {
			...
			ECFieldElementFp.prototype.divide = function divide(b) {
				return new ECFieldElementFp(this.q, this.x.multiply(b.toBigInteger().modInverse(this.q)).mod(
					this.q));
			};
		...
		var ECPointFp = function() {
			...
			ECPointFp.prototype.equals = function equals(other) {
				...
				// u = y2 * z1 - y1 * z2
				var u = other.y.toBigInteger().multiply(this.z).subtract(this.y.toBigInteger().multiply(other
					.z)).mod(this.curve.q);
				if (!u.equals(BigInteger.ZERO)) return false;

				// v = x2 * z1 - x1 * z2
				var v = other.x.toBigInteger().multiply(this.z).subtract(this.x.toBigInteger().multiply(other
					.z)).mod(this.curve.q);
			...
			ECPointFp.prototype.add = function add(b) {
				...
				return new ECPointFp(this.curve, this.curve.fromBigInteger(x3), this.curve.fromBigInteger(y3),
					z3);
			};
			...
			ECPointFp.prototype.twice = function twice() {
				...
				var y3 = w1.multiply(w4.shiftLeft(2).subtract(w6)).subtract(w5.shiftLeft(1).multiply(w3)).mod(
					q);
			...
			ECPointFp.prototype.getMid = function getMid(k3) {
				var neg = this.negate();
				var Q = this;
			...
			ECCurveFp.prototype.decodePointHex = function decodePointHex(s) {
				switch (parseInt(s.substr(0, 2), 16)) {
					...
					case 7:
						...
						return new ECPointFp(this, this.fromBigInteger(new BigInteger(xHex, 16)), this
							.fromBigInteger(new BigInteger(yHex, 16)));
					default:
						// 不支持
						return null;
				}
	...
	var SM2Util = (function() {
		var BigInteger = jsbn.BigInteger,
			SecureRandom = jsbn.SecureRandom;
	...
	var SM3Digest1 = (function() {
		...
		var SM3Digest = function() {
			function SM3Digest() {
				...
				this.v0 = [0x7380166f, 0x4914b2b9, 0x172442d7, 0xda8a0600, 0xa96f30bc, 0x163138aa, 0xe38dee4d,
					0xb0fb0e4e
				];
				this.v0 = [0x7380166f, 0x4914b2b9, 0x172442d7, -628488704, -1452330820, 0x163138aa, -477237683, -
					1325724082
				];
				this.v = new Array(8);
				this.v_ = new Array(8);
			...
			SM3Digest.prototype.processBlock = function processBlock() {
				...
				for (i = 16; i < 68; i++) {
					ww[i] = this.p1(ww[i - 16] ^ ww[i - 9] ^ this.rotate(ww[i - 3], 15)) ^ this.rotate(ww[i -
					13], 7) ^ ww[i - 6];
				}
				for (i = 0; i < 64; i++) {
					ww_[i] = ww[i] ^ ww[i + 4];
				...
				for (i = 0; i < 16; i++) {
					...
					SS2 = SS1 ^ aaa;
					TT1 = Int32.parse(Int32.parse(this.ff_00_15(vv_[0], vv_[1], vv_[2]) + vv_[3]) + SS2) + ww_[
						i];
					TT2 = Int32.parse(Int32.parse(this.gg_00_15(vv_[4], vv_[5], vv_[6]) + vv_[7]) + SS1) + ww[
						i];
					vv_[3] = vv_[2];
				...
				for (i = 16; i < 64; i++) {
					...
					SS1 = this.rotate(SS1, 7);
					SS2 = SS1 ^ aaa;
					TT1 = Int32.parse(Int32.parse(this.ff_16_63(vv_[0], vv_[1], vv_[2]) + vv_[3]) + SS2) + ww_[
						i];
					TT2 = Int32.parse(Int32.parse(this.gg_16_63(vv_[4], vv_[5], vv_[6]) + vv_[7]) + SS1) + ww[
						i];
					vv_[3] = vv_[2];
					vv_[2] = this.rotate(vv_[1], 9);
					vv_[1] = vv_[0];
			...
			SM3Digest.prototype.getZ = function getZ(g, publicKey, userId) {
				...
				if (userId) {
					if (typeof userId !== 'string') throw new Error(
						...
						'undefined' ? 'undefined' : _typeof(userId)));
					if (userId.length >= 8192) throw new Error(
	...
	var sm2 = (function() {
		...
		var SM3Digest = SM3Digest;
		var SM2Cipher = SM2Cipher11;
	...
	var sm3 = (function() {
		...
		function CF(V, Bi) {
			...
			for (var j = 16; j < 68; j++) {
				W.push(calMulti(xor)(P1(calMulti(xor)(W[j - 16], W[j - 9], rol(W[j - 3], 15))), rol(W[j - 13], 7),
					W[j - 6]));
			}
	...
	var sm4 = (function() {
		...
		function arrayToUtf8(arr) {
			...
			for (var i = 0, len = arr.length; i < len; i++) {
				if (arr[i] >= 0xf0 && arr[i] <= 0xf7) {
					// 四字节
					str.push(String.fromCodePoint(((arr[i] & 0x07) << 18) + ((arr[i + 1] & 0x3f) << 12) + ((arr[i +
					2] & 0x3f) << 6) + (arr[i + 3] & 0x3f)));
					i += 3;
				} else if (arr[i] >= 0xe0 && arr[i] <= 0xef) {
					// 三字节
					str.push(String.fromCodePoint(((arr[i] & 0x0f) << 12) + ((arr[i + 1] & 0x3f) << 6) + (arr[i +
					2] & 0x3f)));
					i += 2;
		 ...
		 */
		function byteSub(a) {
			return (Sbox[a >>> 24 & 0xFF] & 0xFF) << 24 | (Sbox[a >>> 16 & 0xFF] & 0xFF) << 16 | (Sbox[a >>> 8 &
			0xFF] & 0xFF) << 8 | Sbox[a & 0xFF] & 0xFF;
		}
	...
	return {
		...
		sm4: sm4,
		SMUtil: SMUtil,
		SM2Cipher11: SM2Cipher11,
		SM3Digest1: SM3Digest1,
		SM2Util: SM2Util,
		EC: EC,
		ASN: ASN
	}
